...
static int hash_delivery(void *phy_addr, int size) {
  unsigned long long addr = (unsigned long long)phy_addr;
...
static int cma_buf_hash_add(cma_buffer_hash_map *map, cma_buffer *data) {
  int hash_index = hash_delivery((unsigned int*)data->phy_addr, map->size);
...
static int cma_buf_hash_find(cma_buffer_hash_map *map, void *phy_addr, cma_buffer *data) {
  int hash_index = hash_delivery((unsigned int*)phy_addr, map->size);
  cma_buffer_hash *cur_node = map->buckets[hash_index];

  while(cur_node != NULL) {
    if ((unsigned int*)cur_node->data.phy_addr == (unsigned int*)phy_addr) {
...
static int cma_buf_hash_remove(cma_buffer_hash_map *map, void *phy_addr) {
  int hash_index = hash_delivery(phy_addr, map->size);
  cma_buffer_hash *cur_node = map->buckets[hash_index];
...
int aic_cma_buf_find(void *phy_addr, cma_buffer *back) {
  return cma_buf_hash_find(g_hash_map, phy_addr, back);
}

int aic_cma_buf_del(void *phy_addr) {
  int ret = -1;
  cma_buffer node = { 0 };
  void *buf_node = NULL;

  ret = aic_cma_buf_find(phy_addr, &node);
  if (ret < 0) {
    log_error("node is not exit, phy_addr = 0x%08x\n", (unsigned int)phy_addr);
    return -1;
  }

  buf_node = (void *)node.phy_addr;
...
void aic_cma_buf_debug(int flag) {
  ...
  if (flag & AIC_CMA_BUF_DEBUG_SIZE) {
    log_debug("remain size = %d, cma_size = %d\n",
    ...
    for (i = 0; i < g_hash_map->size; i++) {
      ...
      while(cur_node != NULL) {
        if (cur_node->data.type == FD_TYPE) {
          log_debug("fd = %d, buf_head = 0x%08x, size = %d\n",
                    cur_node->data.fd,
                    (unsigned int)cur_node->data.buf_head,
                    cur_node->data.size);
        } else if (cur_node->data.type == PHY_TYPE) {
          log_debug("phy = 0x%08x, buf_head = 0x%08x, size = %d\n",
                    (unsigned int)cur_node->data.phy_addr,
                    (unsigned int)cur_node->data.buf_head,
                    cur_node->data.size);
        } else {
          ctx_data = cur_node->data.data;
          log_debug("name = %s, phy = 0x%08x, size = %d\n",
                    ctx_data->name, (unsigned int)cur_node->data.phy_addr,
