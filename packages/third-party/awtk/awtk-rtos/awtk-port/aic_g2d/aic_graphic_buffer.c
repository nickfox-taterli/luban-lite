...
static graphic_buffer_t* graphic_buffer_aic_create(uint32_t w, uint32_t h,
  ...
  size = TK_ROUND_TO(size , CACHE_LINE_SIZE) + CACHE_LINE_SIZE; /* cache line align */

  ret = aic_cma_buf_malloc(&buffer->cma_buff, size, 0);
  if (ret == 0) {
...
graphic_buffer_t* graphic_buffer_aic_create_with_data(void* data) {
  ...
  buffer->data_head = (unsigned int*)dec_asset->frame.buf.phy_addr[0];
