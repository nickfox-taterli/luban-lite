...
int aic_cma_buf_add_ge(cma_buffer *data) {
  ...
  if (ret < 0) {
    log_error("in aic_cma_buf_add_ge, cma_buf_hash_find err, buf = 0x%08x\n", data->phy_addr);
    return -1;
  }
...
int aic_cma_buf_find_ge(void *phy_addr, cma_buffer *back) {
  return aic_cma_buf_find(phy_addr, back);
}

int aic_cma_buf_del_ge(void *phy_addr) {
  aic_cma_buf_del(phy_addr);
  return 0;
}
...
ret_t g2d_fill_rect(bitmap_t* fb, const rect_t* dst, color_t c) {
  ...
  if (ret < 0) {
    ...
    log_debug("in fill, can't find cma mem, dst_data = 0x%08x\n", (unsigned int)dst_data);
#endif
    goto FILL_ERR;
  }
  ...
  aicos_dcache_clean_invalid_range((unsigned long *)dst_buf.phy_addr, (unsigned long)dst_buf.size);
...
static ret_t aic_g2d_bltlet(bitmap_t* dst, bitmap_t* src,
  ...
  aicos_dcache_clean_invalid_range((unsigned long *)src_buf.phy_addr, (unsigned long)src_buf.size);
  aicos_dcache_clean_invalid_range((unsigned long *)dst_buf.phy_addr, (unsigned long)dst_buf.size);
...
static ret_t aic_bitmap_init(bitmap_t* bitmap, bitmap_format_t format,
  ...
  if (ret == RET_FAIL) {
    log_debug("in aic_bitmap_init, aic_bitmap_fmt_check err, format = %d\n", format);
    return RET_FAIL;
  }
...
static ret_t aic_g2d_image_transform(struct mpp_frame *frame, bitmap_t* image,
  ...
  ret = aic_cma_buf_find_ge(dst_data, &dst_buf);
  if (ret < 0) {
    log_debug("aic_cma_buf_find_ge error, dst_data = 0x%08x\n", dst_data);
    goto IMAGE_TRANSFORM_ERR;
  }
  ...
  /* dst buf */
  if (dst_buf.type == PHY_TYPE) {
    blt.dst_buf.buf_type = MPP_PHY_ADDR;
    blt.dst_buf.phy_addr[0] =  dst_buf.phy_addr;
  } else {
    log_error("In aic_g2d_image_transform, dst_buf.type != PHY_TYPE\n");
    return RET_FAIL;
  }
...
static ret_t get_bitmap_format(aic_dec_asset* dec_asset)
{
  ...
  ret = aic_fmt_to_tk_fmt(lcd_format_get());
  if (ret != RET_FAIL) {
    bitmap_format = ret;
  }
#else
