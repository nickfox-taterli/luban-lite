...
static void on_app_exit(void) {
  ...
  tk_aic_g2d_close();
#ifdef WITH_LCD_FLUSH_AND_SWAP
  aic_cma_buf_del_ge(offline_buf.buf_head);
#endif
  aic_cma_buf_del_ge(online_buffer[0].buf_head);
  aic_cma_buf_del_ge(online_buffer[1].buf_head);
#endif
...
static lcd_t* lcd_rtos_double_framebuffer_create(fb_info_t* fb) {
  ...
  if (ret >= 0) {
    fb->fbmem_offline = offline_buf.buf_head;
    log_info("offline_buf.fd = %d, offline_buf.buf_head = 0x%08x, size = %d\n",
        offline_buf.fd, (unsigned int)offline_buf.buf_head, offline_buf.size);
  }

  aic_cma_buf_add_ge(&offline_buf);
#endif /* WITH_LCD_FLUSH_AND_SWAP */
  online_buffer[0].type = PHY_TYPE;
  online_buffer[0].buf_head = (unsigned char *)fb->fbmem0; /* in an rtos, the physical address is essentially the pointer address. */
  online_buffer[0].phy_addr = (unsigned int)fb->fbmem0;
  online_buffer[0].size = line_length * h;

  aic_cma_buf_add_ge(&online_buffer[0]);
  if (fb_is_2fb(fb)) {
    online_buffer[1].type = PHY_TYPE;
    online_buffer[1].buf_head = (unsigned char *)fb->fbmem0 + (line_length * h);
    online_buffer[1].phy_addr = (unsigned int)(fb->fbmem0 + (line_length * h));
