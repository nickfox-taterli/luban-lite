    ...
    const void *data)
{
    ...
    /* The start block in the partition, default is 0 */
    mount_part->start_block = 0;
    mount_part->end_block = dev->block_end - dev->block_start;
    /*3. mount uffs */
...
static int dfs_uffs_open(struct dfs_fd *file)
{
    ...
    file->data = (void *)(unsigned long)fd;
...
static int dfs_uffs_close(struct dfs_fd *file)
{
    ...
    /* regular file operations */
    fd = (int)(unsigned long)(file->data);
                         ...
                         rt_off_t offset)
{
    ...
    {
        result = uffs_seek((int)(unsigned long)(file->data), offset, USEEK_SET);
        if (result >= 0)
            return offset;
    }
