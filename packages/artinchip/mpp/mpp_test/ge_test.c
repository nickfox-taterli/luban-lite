...
#define CLOCK_IMAGE "/sdmc/clock.bmp"
#define SECOND_IMAGE "/sdmc/second.bmp"
#define BLT_BMP "/sdmc/clock.bmp"
...
static void ge_rotate(char *pwbuf, int blen, int argc, char **argv)
{
    aicos_thread_t thid = NULL;

    thid = aicos_thread_create("ge_rotate", 8096, 0, ge_rotate_thread, argv[1]);
    if (thid == NULL) {
...
static void ge_bitblt(int argc, char **argv)
{
    ...
    void *src_buf = aicos_malloc(MEM_CMA, fsize);
    if (!src_buf) {
