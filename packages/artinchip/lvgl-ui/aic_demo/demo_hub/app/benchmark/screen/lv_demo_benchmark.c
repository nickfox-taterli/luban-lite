...
#else
#define lv_font_benchmark_montserrat_12_compr_az lv_font_montserrat_14
#define lv_font_benchmark_montserrat_16_compr_az lv_font_montserrat_14
#define lv_font_benchmark_montserrat_28_compr_az lv_font_montserrat_14
#endif
...
#if USE_ARRAY_IMG
#define IMG_BENCHMARK_COGWHEEL_RGB          &img_benchmark_cogwheel_rgb
#define IMG_BENCHMARK_COGWHEEL_CHROMA_KEYED &img_benchmark_cogwheel_chroma_keyed
#define IMG_BENCHMARK_COGWHEEL_INDEXED16    &img_benchmark_cogwheel_indexed16
#define IMG_BENCHMARK_COGWHEEL_ALPHA16      &img_benchmark_cogwheel_alpha16
#if LV_COLOR_DEPTH == 16
#define IMG_BENCHMARK_COGWHEEL_ARGB         &img_benchmark_cogwheel_rgb565a8
#else
#define IMG_BENCHMARK_COGWHEEL_ARGB         &img_benchmark_cogwheel_argb
#endif
#else
#define IMG_BENCHMARK_COGWHEEL_RGB          LVGL_PATH(benchmark/img_benchmark_cogwheel_rgb.png)
#define IMG_BENCHMARK_COGWHEEL_CHROMA_KEYED LVGL_PATH(benchmark/img_benchmark_cogwheel_chroma_keyed.png)
#define IMG_BENCHMARK_COGWHEEL_INDEXED16    LVGL_PATH(benchmark/img_benchmark_cogwheel_indexed16.png)
#define IMG_BENCHMARK_COGWHEEL_ALPHA16      LVGL_PATH(benchmark/img_benchmark_cogwheel_alpha16.png)
#define IMG_BENCHMARK_COGWHEEL_ARGB         LVGL_PATH(benchmark/img_benchmark_cogwheel_argb.png)
#endif

static void benchmark_ui_cb(lv_event_t * e);
...
static lv_obj_t *ui_benchmark;
static lv_obj_t *top_ui_btn;
static int bench_run_scene;
static lv_timer_t *ui_timer[2];
...
lv_obj_t *lv_demo_benchmark(void)
{
    ...
    ui_benchmark = lv_obj_create(NULL);
    ...
    lv_obj_add_event_cb(ui_benchmark, benchmark_ui_cb, LV_EVENT_ALL, NULL);
...
extern void keypad_back_home_event_cb(lv_event_t * e);

static void benchmark_ui_cb(lv_event_t * e)
{
    lv_event_code_t code = lv_event_get_code(e);
    if (code == LV_EVENT_SCREEN_UNLOAD_START) {
        if (ui_timer[bench_run_scene])
            lv_timer_del(ui_timer[bench_run_scene]);
        ui_timer[bench_run_scene] = NULL;
        if (top_ui_btn) {
            lv_obj_del(top_ui_btn);
            top_ui_btn = NULL;
        }
        lv_anim_del_all();
    }
...
static void monitor_cb(lv_disp_drv_t * drv, uint32_t time, uint32_t px)
{
    ...
    //    lv_obj_invalidate(ui_benchmark);
}
...
static void restart_cb(lv_event_t * e)
{
    ...
    if (code == LV_EVENT_CLICKED) {
        ...
        if (top_ui_btn) {
            lv_obj_add_flag(top_ui_btn, LV_OBJ_FLAG_HIDDEN);
            lv_obj_del_delayed(top_ui_btn, 10);
            top_ui_btn = NULL;
        }
        lv_obj_clean(ui_benchmark);
        benchmark_init();
        scene_next_task_cb(NULL);
    }
}

static void restart_btn_create()
{
    scene_act = -1;
    opa_mode = true;

    top_ui_btn = lv_btn_create(lv_layer_top());
    lv_obj_set_size(top_ui_btn, LV_HOR_RES * 0.12, LV_VER_RES * 0.12);
    lv_obj_set_align(top_ui_btn, LV_ALIGN_TOP_RIGHT);
    lv_obj_set_pos(top_ui_btn, 0, 0);

    lv_obj_t *label = lv_label_create(top_ui_btn);
    lv_label_set_text(label, "Restart");
    lv_obj_center(label);

    lv_obj_add_event_cb(top_ui_btn, restart_cb, LV_EVENT_ALL, NULL);
}
...
static void generate_report(void)
{
    ...
    lv_obj_set_flex_flow(ui_benchmark, LV_FLEX_FLOW_COLUMN);

    title = lv_label_create(ui_benchmark);
    lv_label_set_text_fmt(title, "Weighted FPS: %"LV_PRIu32, fps_weighted);

    subtitle = lv_label_create(ui_benchmark);
    lv_label_set_text_fmt(subtitle, "Opa. speed: %"LV_PRIu32"%%", opa_speed_pct);

    lv_coord_t w = lv_obj_get_content_width(ui_benchmark);
    lv_obj_t * table = lv_table_create(ui_benchmark);
