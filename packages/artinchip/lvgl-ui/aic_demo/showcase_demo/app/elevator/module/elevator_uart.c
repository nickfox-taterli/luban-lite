...
static struct rt_semaphore elevator_sem;
static rt_mutex_t elevator_mutex = RT_NULL;
...
static rt_err_t elevator_uart_input(rt_device_t dev, rt_size_t size)
{
    if (size > 0)
        rt_sem_release(&elevator_sem);
...
static int uart_add_disp_task(char *str_rx, int data_len)
{
...
static void elevator_elevator_uart_rx_thread_entry(void *parameter)
{
    char ch;
    ...
    while (1)
    {
        while (rt_device_read(elevator_uart, -1, &ch, 1) != 1)
        {
            ...
            if (check_uart_receive_done(&code, ch)) {
                    rt_mutex_take(elevator_mutex, RT_WAITING_FOREVER);
                    uart_add_disp_task(str_rx, cnt);
                    cnt = 0;
                    rt_mutex_release(elevator_mutex);
            }
...
next:
            rt_sem_take(&elevator_sem, RT_WAITING_FOREVER);
        }
    }
}

void elevator_cmd_cb(lv_timer_t *tmr)
{
...
int elevator_uart_main(void)
{
    ...
    rt_sem_init(&elevator_sem, "elevator_sem", 0, RT_IPC_FLAG_FIFO);

    rt_thread_t thread_rx = rt_thread_create("elevator_uart_rx",
                                                elevator_elevator_uart_rx_thread_entry,
                                                RT_NULL,
                                                1024 * 8,
                                                15,
                                                15);
    ...
    elevator_mutex = rt_mutex_create("elevator_mutex", RT_IPC_FLAG_PRIO);
    if (elevator_mutex == RT_NULL) {
        rt_kprintf("create elevator_mutex failed.\n");
        return -1;
    }
